...
# 变异率缩放函数（指数递减）
def scaling_function(t, mu_0=0.1, lambda_val=0.1):
    return mu_0 * np.exp(-lambda_val * t)
...
# 遗传算法主流程（带无义密码子GW，带指数递减变异率）
def genetic_algorithm_with_gw_and_scaling(pop_size, genome_length, generations, gw_length=2, pm=0.1, pc=0.9, scale_lambda=0.1):
    population = initialize_population_with_gw(pop_size, genome_length, gw_length)
    best_fitness_over_time = []
    ...
    for gen in range(generations):
        ...
        # 使用指数递减调整变异率
        mutation_prob_scaled = scaling_function(gen, mu_0=mutation_prob, lambda_val=scale_lambda)
        next_generation = [mutate(ind, mutation_prob_scaled) for ind in next_generation]
...
# 参数设置
POP_SIZE = 100  # 种群大小
GENOME_LENGTH = 30  # 假设基因长度为 40
GW_LENGTH = 2  # 无义密码子部分为 2
GENERATIONS = 300
N = 40  # 执行次数
SCALE_LAMBDA = 0.1  # 缩放参数

# 初始化结果存储
fitness_standard = np.zeros(GENERATIONS)
fitness_with_gw = np.zeros(GENERATIONS)
fitness_with_gw_and_scaling = np.zeros(GENERATIONS)

# 执行带GW的GA和带缩放GW的GA多次并计算平均适应度
for _ in range(N):
    ...
    fitness_with_gw_and_scaling += genetic_algorithm_with_gw_and_scaling(POP_SIZE, GENOME_LENGTH, GENERATIONS, GW_LENGTH, scale_lambda=SCALE_LAMBDA)

# 计算平均值
fitness_standard /= N
fitness_with_gw /= N
fitness_with_gw_and_scaling /= N

# 绘制对比图
plt.plot(fitness_standard, label="Standard GA")
plt.plot(fitness_with_gw, label="GA with Genetic Waste (GW)")
plt.plot(fitness_with_gw_and_scaling, label="GA with GW and Scaling")
plt.xlabel("Generation")
plt.ylabel("Best Fitness For Every Generation")
plt.legend()
plt.title("Comparison of GA Performance with and without Genetic Waste (GW) and with Scaling")
plt.savefig("../assets/img/comparison_gw_vs_standard_vs_gwscaling.jpg")
plt.show()